...
import shutil
from class_query import STU_INFO, FEERECEIPT
from rmsvalidators import *

class FeeRecordsMonth(Frame):
    ...
    def initUI(self):
        ...
        self.Mkey = {4:'APRIL',5:'MAY', 6:'JUNE', 7:'JULY', 8:'AUGUST', 9:'SEPT', 10:'OCT',11:'NOV',12:'DEC',1:'JAN',2:'FEB',3:'MAR'}
        self.Mkey_rv = {'APRIL':4,'MAY':5, 'JUNE':6, 'JULY':7, 'AUGUST':8 ,'SEPT':9, 'OCT':10,'NOV':11,'DEC':12,'JAN':1,'FEB':2,'MAR':3}
        Mlist = []
        for k, m in self.Mkey.items():
            Mlist.append(m)
        self.month_cb = RMSCombobox(self.master, values=Mlist, state='readonly', width=6, **combx_fnt)       
        self.month_cb.grid(row=wrow, column=2, rowspan=1, columnspan=1, sticky='w')
        self.month_cb.current(0)#Mlist = ['APRIL','MAY', 'JUNE', 'JULY', 'AUGUST', 'SEPT', 'OCT','NOV','DEC','JAN','FEB','MAR']
        
        self.month_cb.grid(row=wrow, column=4, rowspan=1, columnspan=1, sticky='w') 
        ...
        self.stud_count = RMS_LABEL(self.master, text="Student's Name :", **lfntfg_bgg)
        self.stud_count.grid(row=wrow, column=1, rowspan=1, columnspan=1, sticky='w')
        self.stud_paid = RMS_LABEL(self.master, text="Student's Name :", **lfntfg_bgg)
        self.stud_paid.grid(row=wrow, column=3, rowspan=1, columnspan=1, sticky='w')
        self.stud_unpaid = RMS_LABEL(self.master, text="Student's Name :", **lfntfg_bgg)
        self.stud_unpaid.grid(row=wrow, column=5, rowspan=1, columnspan=1, sticky='w')
        wrow += 1
        ...
        self.snlcpos = {'row':wrow-3, 'column':8,'columnspan':6,'rowspan':10, 'sticky':'w'}
        ...
        gbd, gbg, gfg, wd = 2, '#b0e0e6', 'black', 7
        self.gcolconf = {
        0:{'idname':'sno','text':'S.N','width':5,'bd':gbd,'bg':gbg,'fg':gfg,'font':lbfont,'relief':'raised','wrow':wrow},
        1:{'idname':'rdate','text':'Date','width':wd,'bd':gbd,'bg':gbg,'fg':gfg,'font':lbfont,'relief':'sunken','wrow':wrow},
        2:{'idname':'name','text':'Name','width':20,'bd':gbd,'bg':gbg,'fg':gfg,'font':lbfont,'relief':'sunken','wrow':wrow},
        3:{'idname':'afee','text':'Add.Fee','width':wd,'bd':gbd,'bg':gbg,'fg':gfg,'font':lbfont,'relief':'sunken','wrow':wrow},
        4:{'idname':'tfee','text':'Tu.Fee','width':wd,'bd':gbd,'bg':gbg,'fg':gfg,'font':lbfont,'relief':'sunken','wrow':wrow},
        5:{'idname':'mfee','text':'Mnt.Fee','width':wd,'bd':gbd,'bg':gbg,'fg':gfg,'font':lbfont,'relief':'sunken','wrow':wrow},
        6:{'idname':'lfee','text':'Late.Fee','width':wd,'bd':gbd,'bg':gbg,'fg':gfg,'font':lbfont,'relief':'sunken','wrow':wrow},
        7:{'idname':'ofee','text':'Oth.Fee','width':wd,'bd':gbd,'bg':gbg,'fg':gfg,'font':lbfont,'relief':'sunken','wrow':wrow},
        8:{'idname':'cfee','text':'Cnv.Fee','width':wd,'bd':gbd,'bg':gbg,'fg':gfg,'font':lbfont,'relief':'sunken','wrow':wrow},
        9:{'idname':'efee','text':'Exam.Fee','width':wd,'bd':gbd,'bg':gbg,'fg':gfg,'font':lbfont,'relief':'sunken','wrow':wrow},
        10:{'idname':'add1','text':'Add1','width':wd,'bd':gbd,'bg':gbg,'fg':gfg,'font':lbfont,'relief':'sunken','wrow':wrow},
        11:{'idname':'add2','text':'Add2','width':wd,'bd':gbd,'bg':gbg,'fg':gfg,'font':lbfont,'relief':'sunken','wrow':wrow},
        12:{'idname':'total','text':'Total','width':wd,'bd':gbd,'bg':gbg,'fg':gfg,'font':lbfont,'relief':'sunken','wrow':wrow},
        }
       
        header, rows = True, 10
        self.lc = RMSLBN(self.master, self, header, rows, self.gcolconf, **lb_fg_bg)
        self.lcdata = None
        self.lc.grid(row=wrow, column=0, rowspan=1, columnspan=12, sticky='w')
        ...
        self.editcbv = BooleanVar()
        self.editcb = RMSChkBut(self.master, text='- Edit',
                        command=self.OnEdit_CB, variable=self.editcbv, **chk_fg_bg)
        self.editcb.grid(row=wrow, column=8,)
        ...
        self.month_cb.bind('<<ComboboxSelected>>', self.OnMonth_cb)
        ...
        self.wdfiddict = {self.snlc:'snlc', self.lc:'lc',}
        self.SetDateRange()
    ...
    def OnMonth_cb (self, event):
        self.qclick = True
        self.SetDateRange()
        sval = self.stu_name_tx.GetValue().strip()
        ...
        #if self.qclick:
        #    textlist = '00', '00', sval, self.fyear, getid, sval, key_val, section, '0'
        #    self.snlcdata = self.snlc.SETDATA(self.sqry.Student_Data_Fill, textlist, 'name')
        #    self.qclick = False    
        #self.studid = str(self.snlcdata['esid'])
        #self.SNLCShow()
        
        self.OnClass_entry_cb(event)

    def OnClass_entry_cb(self, event):
        self.SetDateRange()
        val = self.class_entry_cb.GetValue()
        key_val = Combox_Val(self, val)
        section = self.section_tx.GetValue()
        ###################################################
        sval = self.stu_name_tx.GetValue().upper().strip()
        getid = 'ledgerID'
        ##Act = '0' ## 0 for Active Students in School
        ##Out = '1' ## 1 for OUT Students in School
        #if self.qclick :
        ...
        ####################################################
        stidrows , nostd = self.sqry.TOTAL_STUDENT_QUERY(key_val, section, '0')
        self.stud_count.SetLabel('Total Students = %s'%nostd)
        
        dfrm, dtod = self.DATE_GET_VALUE()
        paidrows, paidn = self.fercpt.FEE_RECEIVED_COUNT(key_val, section, dfrm, dtod, self.fyear)
        self.stud_paid.SetLabel(' , Paid = %s'%paidn)
        self.stud_unpaid.SetLabel(' , UnPaid = %s'%(int(nostd)-int(paidn)))
        self.DataFill(vals=[val,key_val,section,sval,])
        self.qclick = True
        #STUDENT_FEE_LIST_FILL(self, trows)
    ...
    def OnEdit_CB(self):
        print ('edit function not prepared yet student_fee_records_month.py line 258')
    ...
    def student_name_tx_key(self, event):
        ...
        if self.snlcdata:
            self.studid = str(self.snlcdata['esid'])
            self.DataFill(vals=None, studentID=self.studid)
        if key in ['Return', 'Tab']:
            ...
            if not self.snlcdata:
                ...
                return
            self.studid = str(self.snlcdata['esid'])
            self.stu_name_tx.SetValue(self.snlcdata['name'])
            self.DataFill()
            self.snlc.Hide()
    ...
    def DataFill(self, vals=None, studentID=None):
        if not vals:
            val = self.class_entry_cb.GetValue()
            key_val = Combox_Val(self, val)
            section = self.section_tx.GetValue()
        ...
        textlist = fdate, tdate, key_val, section, sval, self.fyear, studentID
        self.qclick = True
        self.lcdata = self.lc.SETDATA(self.fercpt.NEW_STUDENT_RECORDS_FETCH, textlist, 'name')
...
def main():
    ...
    spnum = 1
    whxy = (sw, sh-70, 0, 0)
    app = FeeRecordsMonth(root, sptag, spnum, buttonidx, parent, whxy, rscr=rscr)    
    root.mainloop()
